...
if uploaded_file and pdf_title_input:
    ...
    if "NCODIGOPJ" not in df.columns:
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            for (ncodigopj, empresa), grupo in grouped:
                ...
                # Empresa en esquina superior izquierda
                pdf.set_xy(10, 10)
                pdf.set_font("Arial", 'B', 12)
                pdf.set_text_color(0, 0, 0)
                pdf.cell(0, 10, f"{empresa}", ln=False, align="L")

                # Título centrado
                pdf.set_xy(50, 10)
                pdf.set_font("Arial", 'B', 15)
                pdf.set_text_color(95, 158, 160)
                pdf.cell(0, 10, pdf_title_input, ln=True, align="C")
                pdf.ln(15)
